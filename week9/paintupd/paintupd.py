...
buttons = pg.Surface((100, 240)) # button panel
font = pg.font.SysFont("Verdana", 15)
cur_color = 'white' # changable current color
# command dictionary
commands = {
...
def right_triangle(screen, cur, end, d, color): 
    ...
    if x1 <= x2: 
        ...
        if y1 < y2: 
            ...
            pg.draw.polygon(screen, color, [(x1, y1), (x1, y1 - dify), (x2, y2)], d)    
# function for drawing the equal-sided triangle(I hope I spelled it right) )
def triangle(color, pos):
    pg.draw.polygon(surf, color, pos, 3)
# function for drawing the rect
...
while running:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            running = False
        # here we just change the color or the functionality depending on what key the user presses             
        if event.type == pg.KEYDOWN:
            ...
            if event.key == pg.K_5: 
                ...
                for k, v in di.items(): 
                    if k != 'eraser': 
                        di[k] = False   
        # when clicked on a button we set it as True in command dictionary           
                ...
                if v[0] <= pos[0]-700 <= v[0] + v[2] and v[1] <= pos[1] <= v[1] + v[3]:
                    ...
                    for i, j in di.items():
                        if i != k:
                            di[i] = False
                    break
        # here we start the each function depending on what event we clicked on                 
